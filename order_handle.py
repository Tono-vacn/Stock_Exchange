...
def match_given_order(tran_id):
  ...
  cur_order = session.query(Status).filter(Status.transaction_id == tran_id, 
  ...
  for order in cur_match:
    ...
    if abs(order.shares) == abs(cur_order.shares):
      ...
      if cur_order.shares > 0:
        ...
        cur_match_acc.balance += (order_status.price)*abs(cur_order.shares)
      else:
        ...
        cur_match_acc.balance -= (order_status.price-cur_transc.limit)*abs(cur_order.shares)
      session.commit()
      break
      ...
      if cur_order.shares > 0:
        cur_acc.balance -= (order_status.price - cur_match_transc.limit)*abs(cur_order.shares)
        ...
        cur_match_acc.balance -= (order_status.price-cur_match_transc.limit)*abs(cur_order.shares)
      session.commit()
      break
    else:
      order_status = session.query(Status).filter(Status.status_id == order.status_id).with_for_update().first()
      ...
      if cur_order.shares > 0:
        ...
        cur_match_acc.balance += (order_status.price)*abs(order_status.shares)
      else:
        ...
        cur_match_acc.balance -= (order_status.price-cur_match_transc.limit)*abs(order_status.shares)
        session.commit()
  session.close()
  pass
